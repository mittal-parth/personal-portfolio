...
import { aboutMe, itemsToFetch, includedRepos } from "../src/constants";
...
//Function makes another api call if error occurs
async function fetchWithRetry(url, options, retries = 1){
  for(let attempt = 0; attempt <= retries; attempt++){
    try{
      ...
      if(attempt < retries){
        console.warn("Retrying fetching data from Github API", error.message);
      }else{
        throw error;
      }
    }
...
export async function fetchContributions() {
  ...
  try {
    const response = await fetchWithRetry("https://api.github.com/search/issues", {
      params: {
        q: `author:${aboutMe.githubUsername} repo:${includedRepos.join(" repo:")}`,
        sort: "created",
        order: "desc",
